...
public class Asteroids : MonoBehaviour
{
    ...
    // Components
    public RawImage raw;
    public RawImage outlines;
    public RawImage hexagons;
    public RawImage hexagons_outlines;
    private RectTransform rect;

    public GameObject solar_system_obj;
    ...
    void Update()
    {
        ...
        // Draw Dungeon
        //drawHexagonDungeon(dungeon);
        hexagons.GetComponent<RectTransform>().localPosition = new Vector3(Mathf.RoundToInt(-camera_position.x), Mathf.RoundToInt(-camera_position.y), 0f);
        hexagons_outlines.GetComponent<RectTransform>().localPosition = new Vector3(Mathf.RoundToInt(-camera_position.x), Mathf.RoundToInt(-camera_position.y), 0f);
    ...
    private void drawShip(ShipBehaviour ship) {
        ...
        // Ship Compass
        if (Vector2.Distance(ship.position, Vector2.zero) > 1000) {
            float compass_direction = Mathf.Atan2(-ship.position.y, -ship.position.x);
            Vector2 compass_offset = new Vector2(Mathf.Cos(compass_direction), Mathf.Sin(compass_direction)) * 100;
            drawCircle(temp_x + Mathf.RoundToInt(compass_offset.x), temp_y + Mathf.RoundToInt(compass_offset.y), 6, 120, compass_direction * Mathf.Rad2Deg, Color.white);
        }
    ...
    private void updateScreen() {
        if (update_scale) {
            ...
            outlines.GetComponent<RectTransform>().sizeDelta = new Vector2(screen_width, screen_height);
            outlines.GetComponent<RectTransform>().localScale = new Vector2(screen_scale, screen_scale);
        }
